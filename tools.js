...
export function ToolColor(canvas) {
  var isPress = false;
  ...
  this.onMouseMove = function(e) {
    ...
    color.setColorText(
      this.rgbToHex(pixel.data[0], pixel.data[1], pixel.data[2])
    );
    color.draw(canvas.getRender());
  };
}

export function ToolRuler(canvas) {
  var isPress = false;
  ...
  this.onMouseMove = function(e) {
    ...
    if (isPress) {
      var isVertical =
        Math.abs(p.x - ruler.getP1().x) > Math.abs(p.y - ruler.getP1().y)
          ? false
          : true;
      if (isVertical) {
...
export function ToolCoord(canvas) {
  var isPress = false;
...
export function ToolPan(canvas) {
  var isPress = false;
  var ox = 0;
  var oy = 0;
